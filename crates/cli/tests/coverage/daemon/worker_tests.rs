...
fn worker_network_must_match_activation_grant() {
    ...
    let bind_mismatch = Options {
        port: None,
        ..options.clone()
    };
    assert!(validate_bootstrap(&bind_mismatch, &bootstrap(Ipv4Addr::UNSPECIFIED, 0)).is_err());
    ...
    let mut advertised_grant = bootstrap(Ipv4Addr::UNSPECIFIED, 0);
    advertised_grant.advertise_address = Some("worker.example.com".into());
    assert_eq!(
        validate_bootstrap(&advertised, &advertised_grant).unwrap(),
