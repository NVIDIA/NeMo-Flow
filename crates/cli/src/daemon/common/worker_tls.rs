...
use super::transport::{CONNECT_TIMEOUT, PooledClient, pooled_builder, pooled_worker_h2c_client};
...
impl WorkerTlsIdentity {
    ...
    pub(crate) fn generate(advertised_host: &str) -> Result<Self, CliError> {
        ...
        let invalid_ip = advertised_host
            .parse::<std::net::IpAddr>()
            .is_ok_and(|address| address.is_unspecified() || address.is_multicast());
        if advertised_host.is_empty() || invalid_ip {
