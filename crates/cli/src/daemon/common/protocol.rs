...
impl ComponentDescriptor {
    ...
    pub(crate) fn validate(&self) -> Result<(), ProtocolError> {
        ...
        if self.binary_version.len() > MAX_BINARY_VERSION_BYTES {
            return Err(ProtocolError::BinaryVersionTooLong);
        }
        Ok(())
    }
}
...
pub(crate) enum ProtocolError {
    ...
    #[error("the component binary version exceeds the protocol limit")]
    BinaryVersionTooLong,
