...
use super::common::address::{explicit_daemon_origin, worker_advertised_address, worker_socket};
...
pub(crate) async fn run(options: Options) -> Result<(), CliError> {
    ...
    let worker_tls = if local.ip().is_unspecified() {
        let advertised_host = options.advertise_address.as_deref().ok_or_else(|| {
            CliError::Unauthorized(
                "an unspecified worker bind requires an advertised address".into(),
            )
        })?;
        Some(WorkerTlsIdentity::generate(advertised_host)?)
    } else {
        None
    };
    let endpoint = format!(
