...
const CODEX_ARTIFACT_PATHS: &[&str] = &[
...
const CLAUDE_ARTIFACT_PATHS: &[&str] = &[
...
fn render_codex(spec: &ManagedBundleSpec) -> Result<Vec<RenderedArtifact>, CliError> {
    ...
    Ok(vec![
        artifact(
            ManagedAgent::Codex,
            CODEX_ARTIFACT_PATHS[0],
            json_bytes(&plugin)?,
        ),
        artifact(
            ManagedAgent::Codex,
            CODEX_ARTIFACT_PATHS[1],
            json_bytes(&mcp)?,
        ),
        artifact(
            ManagedAgent::Codex,
            CODEX_ARTIFACT_PATHS[2],
            hook_bytes(ManagedAgent::Codex, CODEX_HOOK_EVENTS, spec)?,
        ),
        artifact(
            ManagedAgent::Codex,
            CODEX_ARTIFACT_PATHS[3],
            settings.into_bytes(),
        ),
    ])
}
...
fn render_pi(spec: &ManagedBundleSpec) -> Result<Vec<RenderedArtifact>, CliError> {
    ...
    Ok(vec![
        ...
        artifact(ManagedAgent::Pi, PI_ARTIFACT_PATHS[2], config),
        artifact(
            ManagedAgent::Pi,
            PI_ARTIFACT_PATHS[3],
...
fn toml_string(value: &str) -> String {
    assert!(
        ...
        "managed TOML values must be validated ASCII"
    );
    format!("{value:?}")
}
...
fn all_artifact_paths() -> BTreeSet<&'static str> {
    std::iter::once(MANIFEST_FILE)
        .chain(
            ...
            .into_iter()
            .flat_map(|agent| artifact_paths_for(agent).iter().copied()),
        )
        .collect()
}

fn artifact_paths_for(agent: ManagedAgent) -> &'static [&'static str] {
    match agent {
        ManagedAgent::Codex => CODEX_ARTIFACT_PATHS,
        ManagedAgent::ClaudeCode => CLAUDE_ARTIFACT_PATHS,
        ManagedAgent::Pi => PI_ARTIFACT_PATHS,
    }
}
