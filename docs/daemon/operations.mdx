...
| Control connection fails through a proxy | WebSocket upgrade or idle-session handling | Allow both v2 upgrade paths and check [proxy requirements](/daemon/reference#configure-a-trusted-reverse-proxy). |
| Control protocol version is rejected | Incompatible daemon and client binaries | Install v2-compatible daemon, MCP, and worker binaries together; there is no v1 fallback. |
...
definition, configuration, and separately trusted bundle digest. Check that the
daemon and every client's MCP and worker binaries support control protocol v2.
A v1/v2 mixed deployment cannot recover through a legacy control endpoint.
...
alone is not the compatibility check. A rollback across the v2 boundary requires
rolling back all daemon and client components together.
