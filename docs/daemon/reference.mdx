...
MCP clients and workers each open an authenticated WebSocket control session to
the daemon. Protocol v2 carries registration, recovery, readiness, launch
instructions, release, and drain messages. Upgrade the daemon, MCP clients, and
workers together; v1 HTTP control endpoints are no longer supported. Provider
and hook requests still use HTTP, and MCP clients still serve stdio locally.

The upgrade paths are `/_nemo-relay/control/v2/mcp` and
`/_nemo-relay/control/v2/worker`. Clients derive `ws` or `wss` from the configured
HTTP or HTTPS daemon origin. Each new connection authenticates before it can
change state. Commands carry request IDs and receive explicit acknowledgments;
pushed directives are received independently of command replies. A replaced
connection cannot invalidate its successor. Each control command also has a
five-second deadline, including time waiting to be sent.

Control messages are limited to 256 KiB. Outbound queues and client event queues
hold at most 1,024 messages; clients also bound outstanding replies to 1,024. Queue
overflow closes the connection and triggers normal recovery. These limits are
independent of HTTP body streaming and have no CLI configuration.
...
- Allow WebSocket upgrades on `/_nemo-relay/control/v2/mcp` and
  `/_nemo-relay/control/v2/worker`, including bidirectional frames after upgrade.
