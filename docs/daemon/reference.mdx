...
send no periodic pings. For non-loopback connections, the daemon sends a ping
every 30 seconds and requires its matching pong within 10 seconds. An idle hung
loopback process may remain undetected until an operation times out.

After a disconnect, clients retry immediately with backoff capped at two
seconds. Each connection and authentication attempt has a five-second timeout,
...
a separately connected worker can continue serving it.
