...
| Execution intercept with context | `register_tool_execution_v2` | Not applicable | Yes |
...
A tool execution intercept registered through the `_v2` family receives a
`ToolExecutionContext` — carrying the tool name, the arguments, and the managed
`tool_call_id` — in place of the tool name and arguments. Use it when an
intercept short-circuits execution and must still correlate its result with the
originating tool call. Both families share one registry, so they order together
by priority and `deregister_tool_execution` removes either. See
[Tool Execution Intercept Outcomes](/reference/tool-execution-intercept-outcomes)
for the per-binding names.
