...
<Warning title="Breaking change: tool execution intercept callbacks">

Tool execution intercept callbacks now receive `(ToolExecutionContext, next)`.
`ToolExecutionContext` contains the tool name, incoming arguments, and optional
`tool_call_id`.

Update existing callbacks and declare a Relay 0.9-compatible plugin range before
upgrading. See the [0.9 migration guidance](/reference/migration-guides).
