...
A worker can register a tool execution intercept through
`register_tool_execution_intercept_v2` instead, whose callback receives
`(context, next_call)`. The `ToolExecutionContext` carries `tool_name`,
`arguments`, and the managed `tool_call_id`, which the host sends in the
additive `ToolInvocation.tool_call_id` protobuf field. Use it when a worker
