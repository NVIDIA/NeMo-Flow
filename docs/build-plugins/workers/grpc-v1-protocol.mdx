...
additive: workers built before it existed ignore it, and hosts that predate it
never set it, so no `grpc-v1` version change is required. Worker SDKs surface it
through their context-carrying registration
