...
Beginning with Relay 0.9, each binding's existing tool execution registration
receives a `ToolExecutionContext` and the continuation. The context exposes the
...
its result with the originating tool call. See
[Tool Execution Intercept Outcomes](/reference/tool-execution-intercept-outcomes)
for the full field list and migration contract.
