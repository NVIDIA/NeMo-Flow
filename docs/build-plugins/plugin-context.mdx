...
Each binding also exposes a context-carrying registration —
`register_tool_execution_intercept_v2`, `registerToolExecutionInterceptV2`, or
`RegisterToolExecutionInterceptV2` — whose callback receives a
`ToolExecutionContext` instead of the arguments alone. The context exposes the
tool name, the arguments entering the intercept, and the managed `tool_call_id`,
which a plugin needs when it completes execution itself and must still associate
its result with the originating tool call. Both shapes share one registry and
one deregistration. See
