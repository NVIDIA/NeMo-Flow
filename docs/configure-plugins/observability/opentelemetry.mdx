...
| `gen_ai_capture_tool_content` | `false` | Opt into sanitized tool arguments, successful object results, and minimal inference tool definitions for `gen_ai`. |
...
content with an LLM or event sanitizer. Tool payloads are omitted by default;
...
Set `gen_ai_capture_tool_content = true` on an individual `gen_ai` trace
endpoint to opt into sanitized `gen_ai.tool.call.arguments` at tool start,
`gen_ai.tool.call.result` on successful completion, and `gen_ai.tool.definitions`
on inference spans. These attributes can contain sensitive information.
This setting is independent of `enable_full_payloads` and has no effect on
other projection types. Credential removal and event sanitizers remain active.
...
`type` and `name` properties; definitions without a usable name are omitted.
...
type = "gen_ai"
endpoint = "http://localhost:4318/v1/traces"
gen_ai_capture_tool_content = true
```
