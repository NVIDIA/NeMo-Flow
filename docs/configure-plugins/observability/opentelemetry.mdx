...
GenAI endpoints require HTTPS for remote collectors, for both HTTP and gRPC
transport. HTTP is permitted only for `localhost` and loopback IP addresses
such as `127.0.0.1` and `[::1]` for local collectors. Redaction does not disable
...
start, without replacing explicitly supplied metadata. Generic gateway sessions
