    ...
    pub fn from_file(
        ...
        for target in target_map.values() {
            if !target.headers.is_empty() {
                validate_credential_target(&target.url)?;
            }
        }
...
fn validate_url(url: &str) -> Result<()> {
    let uri = http::Uri::from_str(url).context("invalid target URL")?;
        ...
        "target URL must not contain userinfo"
    );
    ensure!(
        uri.path_and_query()
...
fn validate_credential_target(url: &str) -> Result<()> {
    let uri = http::Uri::from_str(url).context("invalid target URL")?;
    if uri.scheme_str() != Some("http") {
        return Ok(());
    }
    let host = uri.host().context("target URL must contain a host")?;
    let loopback = host.eq_ignore_ascii_case("localhost")
        ...
        loopback,
        "credential-bearing benchmark targets must use HTTPS unless they are loopback"
    );
    Ok(())
}
